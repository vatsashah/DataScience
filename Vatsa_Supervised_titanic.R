...
ggplot(train,aes(Sex,Age))+
  geom_point(aes(color=factor(Survived),size=factor(Survived)))+
...
Survived_formula=as.formula(equation)
...
write.csv(output, file="Vatsa_titanic_Supervised_Ans.csv",row.names = FALSE)
